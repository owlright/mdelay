...
static void parse_options(int argc, char** argv, struct configuration* cfg)
{
    int option_index = 0;
    int opt;
    static struct option long_options[] = { { "ioctl", required_argument, 0, 'i' },
        { "port", required_argument, 0, 'p' }, { "proto", required_argument, 0, 'P' },
        { "max", required_argument, 0, 'n' }, { "ext", no_argument, 0, 'e' }, { 0, no_argument, 0, 0 } };
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < total_received; ++i) {
        fprintf(f, "%ld,%ld,%ld\n", nic_user_latency_numbers[i], nic_kernel_latency_numbers[i],
            kernel_user_latency_numbers[i]);
    }
    fclose(f);
